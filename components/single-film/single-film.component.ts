...
export class SingleFilmComponent implements OnInit {
  ...
  showAll(index, twoD, threeD, selected: number) {
    ...
    this.typeMain = this.sessions[index].hall.typeWatch;
    ...
    if (this.type[selected] === 'Студентський' && this.sessions[index].hall.typeWatch === '2D') {
      this.loginValue.prise = this.twoD * 0.7;
    }
    if (this.type[selected] === 'Дитячий' && this.sessions[index].hall.typeWatch === '2D') {
      this.loginValue.prise = this.twoD * 0.5;
    }
  ...
  getIndex(selected: number) {
    ...
    if (this.type[selected] === 'Повний' && this.typeMain === '3D') {
      this.loginValue.prise = this.threeD;
    }
    if (this.type[selected] === 'Студентський' && this.typeMain === '3D') {
      this.loginValue.prise = this.threeD * 0.7;
    }
    if (this.type[selected] === 'Дитячий' && this.typeMain === '3D') {
      this.loginValue.prise = this.threeD * 0.5;
    }
    if (this.type[selected] === 'Повний' && this.typeMain === '2D') {
      this.loginValue.prise = this.twoD;
    }
    if (this.type[selected] === 'Студентський' && this.typeMain === '2D') {
      this.loginValue.prise = this.twoD * 0.7;
    }
    if (this.type[selected] === 'Дитячий' && this.typeMain === '2D') {
      this.loginValue.prise = this.twoD * 0.5;
    }
  ...
  getId(id: any) {
    ...
    console.log(id, 22);
    setTimeout(() => this.route.navigate([`ticket/${id - 1}`]), 1000);
